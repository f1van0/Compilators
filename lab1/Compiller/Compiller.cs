...
namespace lab1.Compiller
{
    ...
    {
        public static Action<Identifier, Identifier, ICallable, List<Identifier>> Evaluated;
        /// <summary>
        ...
        public static List<List<string>> LexicalAnalize(string code)
        {
        ...
        public static List<List<ILexem>> SemanticAnalize(List<List<string>> parts, out HashSet<ILexem> Identifiers)
        {
            List<ILexem> Lexems;
        ...
        public static List<ILexem> SyntaxAnalize(List<string> expression)
        {
		...
		/// <returns></returns>
		static public Identifier EvaluatePart(List<ILexem> Postfix)
        {
        ...
        static public Identifier Evaluate(List<ILexem> Postfix)
        {
            ...
            foreach (var item in Postfix)
            {
                ...
                else
                {
                    ...
                    string strArgs = String.Join("", args.Select(a => a.PostfixName).ToList());
                    var temp = new Identifier($"T{k++}", $"{strArgs}{item.Literal}", result);
                    Evaluated?.Invoke(temp, op1, func, parameters.ToList());
                    buffer.Push(temp);
                }
            }
            var res = (Identifier)buffer.Pop();
